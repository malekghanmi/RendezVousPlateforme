...
<%
    // 1. SI UN MÉDECIN EST DÉJÀ CONNECTÉ -> Redirection directe vers son espace
    ...
    // 2. Récupération du patient pour le menu
...
<head>
    <meta charset="UTF-8">
    <title>Accès Médecin - Smile Everyday</title>
    ...
    <style>
        ...
        .login-card {
            ...
            border-radius: 20px;
            max-width: 400px; margin: 180px auto 50px auto; 
            padding: 40px;
        ...
        h1 { margin-top: 0; color: #2c3e50; font-size: 2em; margin-bottom: 30px; font-weight: bold; }
        
        .icon-doc { font-size: 50px; margin-bottom: 10px; display: block; }
        ...
        input:focus { border-color: #3498db; outline: none; box-shadow: 0 0 8px rgba(52, 152, 219, 0.3); }

        .btn-row { display: flex; gap: 10px; margin-bottom: 25px; margin-top: 25px; }
        
        /* Bouton Bleu pour le médecin */
        .btn-blue { flex: 1; padding: 12px; background: linear-gradient(45deg, #3498db, #2980b9); color: white; border: none; border-radius: 50px; cursor: pointer; font-weight: bold; font-size: 16px; transition: 0.3s; }
        .btn-blue:hover { transform: scale(1.05); background: linear-gradient(45deg, #2980b9, #1f618d); }
        ...
        .error-msg { color: #e74c3c; background: #fadbd8; padding: 12px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #e6b0aa; font-weight: bold; font-size: 0.9em; }
...
<body>
    ...
    <div class="header">
        ...
        <ul class="nav-links">
            ...
            <li><a href="connexionDentiste.jsp" class="active" style="color: #3498db;">Médecin</a></li>
        </ul>
    ...
    <div class="login-card">
        <span class="icon-doc">👨‍⚕️</span>
        <h1>Espace Médecin</h1>

        <% if (request.getParameter("error") != null) { %>
            <div class="error-msg">⚠️ Identifiants incorrects</div>
        <% } %>

        <form action="${pageContext.request.contextPath}/Controleur" method="post">
            <input type="hidden" name="action" value="connexionDentiste">

            <div class="input-group">
                <label>Email Professionnel</label>
                <input type="email" name="email" placeholder="dr.nom@smile.com" required>
            </div>

            <div class="input-group">
                <label>Mot de passe</label>
                <input type="password" name="mdp" placeholder="Votre code d'accès" required>
            </div>

            <div class="btn-row">
                <button type="submit" class="btn-blue">Se Connecter</button>
                <button type="button" class="btn-grey" onclick="window.location.href='index.jsp'">Retour</button>
            </div>
